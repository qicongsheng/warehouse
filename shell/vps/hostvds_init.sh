...
docker run -d --restart=always -e API_KEY=LAdoeJAuaq_uB2kCblxZDtF5wJFrDYHQfSHOFge0 -e ZONE=qics.top -e SUBDOMAIN=hostvds ghcr.io/qicongshengddns
docker run -d --restart=always ghcr.io/qicongshengtraffmonetizer start accept --token 5xijhElDM8IqEwMj0VolDEVsbUDCGS3GFpRCFckxix8=
docker run -d --restart=always --name hostvds_ssh_59022 -e SERVER_ADDR=play.qics.top -e PROXY_NAME=hostvds_ssh_59022 -e SERVER_PORT=7000 -e TOKEN=badboy -e LOCAL_PORT=9022 -e REMOTE_PORT=59022 --net=host ghcr.io/qicongshengfrp:client
