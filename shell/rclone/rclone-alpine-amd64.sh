...
if [ ! "$(command -v rclone)" ]; then
    ...
    mkdir -p /opt/rclone/onedriver && mkdir -p /onedriver && mkdir /qiniuoss
    ...
    nohup rclone --config /opt/rclone/rclone.conf mount onedrive:/rclone/data /onedriver --copy-links --no-gzip-encoding --no-check-certificate --allow-other --allow-non-empty --umask 000 > /var/log/onedriver.log 2>&1 &
    nohup rclone --config /opt/rclone/rclone.conf mount qiniuoss:/ /qiniuoss --copy-links --no-gzip-encoding --no-check-certificate --allow-other --allow-non-empty --umask 000 > /var/log/qiniuoss.log 2>&1 &
    echo "rclone安装完成." >&2
fi